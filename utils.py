...
sys.path.append('../')
from solve_gaussian_intervention import solve, compute_A_opt
...
def alt_tqa_evaluate(models, metric_names, input_path, output_path, summary_path, device='cpu', verbose=False, preset='qa', interventions={}, intervention_fn=None, cache_dir=None, separate_kl_device=None, instruction_prompt=True, many_shot_prefix=None, judge_name=None, info_name=None): 
    ...
    for mdl in models.keys(): 
        ...
        # llama
        if mdl in ['llama_7B', 'alpaca_7B', 'vicuna_7B', 'llama2_chat_7B', 'llama2_chat_13B', 'llama2_chat_70B', 'llama3_instruct_8B', 'llama3_8B']:
...
def get_ot_interventions_dict(top_heads, probes, tuning_activations, tuning_labels, best_th, num_heads, save_folder, alpha): 
    ...
    for layer, head in top_heads:
        ...
        save_file_A = os.path.join(save_folder, f"model.layers.{layer}.{head}.self_attn.head_out_A.npy")
        save_file_b = os.path.join(save_folder, f"model.layers.{layer}.{head}.self_attn.head_out_b.npy")
        ...
        interventions[f"model.layers.{layer}.self_attn.head_out"].append((head, A_st, b_st, probes[layer_head_to_flattened_idx(layer, head, num_heads)], best_th))
    for layer, head in top_heads: 
        interventions[f"model.layers.{layer}.self_attn.head_out"] = sorted(interventions[f"model.layers.{layer}.self_attn.head_out"], key = lambda x: x[0])
